          ...
          for (let i = 0; i < Board.length; i++)
            for (let j = 0; j < upLeftArray.length; j++)
              if (Board[i].row - Pawns[upLeftArray[j][1]].row == -1 && Board[i].column - Pawns[upLeftArray[j][1]].column == -1  
                ...
                console.log("nearest", nearest)
                killConditions.push([upLeftArray[upLeftArray.length - 1][0], upLeftArray[upLeftArray.length - 1][1], upLeftArray[upLeftArray.length - 1][2], Pawns[upLeftArray[upLeftArray.length - 1][0]].isRed, Greenturn, Pawns[upLeftArray[upLeftArray.length - 1][0]].rectCenter, Pawns[upLeftArray[upLeftArray.length - 1][0]].rectCenterY, Pawns[upLeftArray[upLeftArray.length - 1][1]].rectCenter, Pawns[upLeftArray[upLeftArray.length - 1][1]].rectCenterY, true, 'up-left']);
                killConditionsUnique = JSON.parse(JSON.stringify(killUnique(killConditions)));
                break;
              }
              break;
          ...
          for (let i = 0; i < Board.length; i++)
            for (let j = 0; j < upRightArray.length; j++)
              if (Board[i].row - Pawns[upRightArray[j][1]].row == 1 && Board[i].column - Pawns[upRightArray[j][1]].column == -1  
                ...
                let nearest = Math.max(...rows);
                console.log("nearest", nearest)
                killConditions.push([upRightArray[upRightArray.length - 1][0], upRightArray[upRightArray.length - 1][1], upRightArray[upRightArray.length - 1][2], Pawns[upRightArray[upRightArray.length - 1][0]].isRed, Greenturn, Pawns[upRightArray[upRightArray.length - 1][0]].rectCenter, Pawns[upRightArray[upRightArray.length - 1][0]].rectCenterY, Pawns[upRightArray[upRightArray.length - 1][1]].rectCenter, Pawns[upRightArray[upRightArray.length - 1][1]].rectCenterY, true, 'up-right']);
                killConditionsUnique = JSON.parse(JSON.stringify(killUnique(killConditions)));
                break;
              }
              break;
  ...
  for (let i = 0; i < killConditionsUnique.length; i++) 
    for (let j = i + 1; j < killConditionsUnique.length; j++)
      if (killConditionsUnique[i][0] != killConditionsUnique[j][0] && 
          ...
          (killConditionsUnique[i][10] == 'up-right' && maxRigt.some(array => array === Pawns[killConditionsUnique[i][1]].row)) ||
          ...
          (killConditionsUnique[j][10] == 'up-right' && maxRigt.some(array => array === Pawns[killConditionsUnique[j][1]].row)) ||
    ...
    for (let i = 0; i < killConditionsUnique.length; i++) {
      for (let j = i + 1; j < killConditionsUnique.length; j++) {
          if (killConditionsUnique[i][0] != killConditionsUnique[j][0] && 
              ...
              let conditionMet = (
                  (!killConditionsUnique[i][9] && killConditionsUnique[j][9] && 
                  ((killConditionsUnique[j][10] === 'up-left' && maxLeft.includes(Pawns[killConditionsUnique[j][1]].row)) ||
                  (killConditionsUnique[j][10] === 'up-right' && maxRight.includes(Pawns[killConditionsUnique[j][1]].row)) ||
                  (killConditionsUnique[j][10] === 'down-left' && minLeft.includes(Pawns[killConditionsUnique[j][1]].row)) ||
                  (killConditionsUnique[j][10] === 'down-right' && minRight.includes(Pawns[killConditionsUnique[j][1]].row)))) ||
  
                  (killConditionsUnique[i][9] && !killConditionsUnique[j][9] && Pawns[killConditionsUnique[i][1]].row == killConditionsUnique[i][10]) ||
  
                  (killConditionsUnique[i][9] && killConditionsUnique[j][9] &&
                  ((killConditionsUnique[i][10] === 'up-left' && maxLeft.includes(Pawns[killConditionsUnique[i][1]].row)) ||
                  (killConditionsUnique[i][10] === 'up-right' && maxRight.includes(Pawns[killConditionsUnique[i][1]].row)) ||
                  (killConditionsUnique[i][10] === 'down-left' && minLeft.includes(Pawns[killConditionsUnique[i][1]].row)) ||
                  (killConditionsUnique[i][10] === 'down-right' && minRight.includes(Pawns[killConditionsUnique[i][1]].row))) &&
                  ((killConditionsUnique[j][10] === 'up-left' && maxLeft.includes(Pawns[killConditionsUnique[j][1]].row)) ||
                  (killConditionsUnique[j][10] === 'up-right' && maxRight.includes(Pawns[killConditionsUnique[j][1]].row)) ||
                  (killConditionsUnique[j][10] === 'down-left' && minLeft.includes(Pawns[killConditionsUnique[j][1]].row)) ||
                  (killConditionsUnique[j][10] === 'down-right' && minRight.includes(Pawns[killConditionsUnique[j][1]].row)))) ||
